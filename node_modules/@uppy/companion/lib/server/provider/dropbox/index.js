...
class DropBox extends Provider {
     ...
     */
    async list(options) {
        return __classPrivateFieldGet(this, _DropBox_instances, "m", _DropBox_withErrorHandling).call(this, 'provider.dropbox.list.error', async () => {
    ...
    async download({ id, token }) {
        return __classPrivateFieldGet(this, _DropBox_instances, "m", _DropBox_withErrorHandling).call(this, 'provider.dropbox.download.error', async () => {
    ...
    async thumbnail({ id, token }) {
        return __classPrivateFieldGet(this, _DropBox_instances, "m", _DropBox_withErrorHandling).call(this, 'provider.dropbox.thumbnail.error', async () => {
    ...
    async size({ id, token }) {
        return __classPrivateFieldGet(this, _DropBox_instances, "m", _DropBox_withErrorHandling).call(this, 'provider.dropbox.size.error', async () => {
            const { size } = await getClient({ token }).post('files/get_metadata', { json: { path: id }, responseType: 'json' }).json();
            return parseInt(size, 10);
        });
    }
    async logout({ token }) {
        return __classPrivateFieldGet(this, _DropBox_instances, "m", _DropBox_withErrorHandling).call(this, 'provider.dropbox.logout.error', async () => {
            await getClient({ token }).post('auth/token/revoke', { responseType: 'json' });
            return { revoked: true };
        });
    }
    async refreshToken({ clientId, clientSecret, refreshToken }) {
        return __classPrivateFieldGet(this, _DropBox_instances, "m", _DropBox_withErrorHandling).call(this, 'provider.dropbox.token.refresh.error', async () => {
...
_DropBox_instances = new WeakSet(), _DropBox_withErrorHandling = 
