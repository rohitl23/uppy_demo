...
class Box extends Provider {
    ...
    async list({ directory, token, query, companion }) {
        return __classPrivateFieldGet(this, _Box_instances, "m", _Box_withErrorHandling).call(this, 'provider.box.list.error', async () => {
    ...
    async download({ id, token }) {
        return __classPrivateFieldGet(this, _Box_instances, "m", _Box_withErrorHandling).call(this, 'provider.box.download.error', async () => {
    ...
    async thumbnail({ id, token }) {
        return __classPrivateFieldGet(this, _Box_instances, "m", _Box_withErrorHandling).call(this, 'provider.box.thumbnail.error', async () => {
    ...
    async size({ id, token }) {
        return __classPrivateFieldGet(this, _Box_instances, "m", _Box_withErrorHandling).call(this, 'provider.box.size.error', async () => {
            const { size } = await getClient({ token }).get(`files/${id}`, { responseType: 'json' }).json();
            return parseInt(size, 10);
        });
    }
    logout({ companion, token }) {
        return __classPrivateFieldGet(this, _Box_instances, "m", _Box_withErrorHandling).call(this, 'provider.box.logout.error', async () => {
...
_Box_instances = new WeakSet(), _Box_withErrorHandling = 
